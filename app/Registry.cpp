...
bool RegWriteStr(HKEY hKey, LPCWSTR KeyName, LPCWSTR Value)
{
    ...
    LSTATUS status = RegSetValueExW(hKey, KeyName, 0, REG_SZ, reinterpret_cast<const BYTE *>(Value), ValueSize);
...
{
    LSTATUS status = RegSetValueExW(hKey, KeyName, 0, REG_DWORD, reinterpret_cast<const BYTE *>(&Value), sizeof(4));
...
bool RegReadStr(HKEY hKey, LPCWSTR KeyName, LPWSTR Buffer, size_t BufferLength)
{
    ...
    LSTATUS status = RegGetValueW(hKey, L"", KeyName, RRF_RT_REG_SZ, nullptr, Buffer, &cbData);
...
bool RegReadDWORD(HKEY hKey, LPCWSTR KeyName, PDWORD dwDword)
{
    ...
    LSTATUS status = RegGetValueW(hKey, L"", KeyName, RRF_RT_REG_DWORD, nullptr, dwDword, &cbData);
